...
`;
  ...
  it('includes final structural migrations in dependency order', () => {
    expect(orderedMigrations.at(-5)).toBe('./migrations/0009_campaign_memberships_role_unification.sql');
    expect(orderedMigrations.at(-4)).toBe('./migrations/0010_drop_campaign_gm_assignments.sql');
    expect(orderedMigrations.at(-3)).toBe('./migrations/0011_content_search_fts.sql');
    expect(orderedMigrations.at(-2)).toBe('./migrations/0012_contributors_and_attributions.sql');
    expect(orderedMigrations.at(-1)).toBe('./migrations/0013_drop_email_canonical.sql');
  });
