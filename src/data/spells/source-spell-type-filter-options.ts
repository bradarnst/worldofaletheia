// The public spell API exposes sourceType as a filter but does not currently
// publish a dedicated source spell type enumeration endpoint. Keep these UI
// filter options source-controlled and separate from the active spell type API
// list so source lineage remains an explicit facet rather than an implicit
// reuse of active spell type metadata.
export const SOURCE_SPELL_TYPE_FILTER_OPTIONS: string[] = [
  'Adventurer Spells',
  'Ageing Spells',
  'Air Spells',
  'Alkahest Spells',
  'Angelic Spells',
  'Animal Spells',
  'Antimagic Spells',
  'Antimatter Spells',
  'Artillery Spells',
  'Assorted Spells',
  'Banefire Spells',
  'Banestorm Spells',
  'Bio-Tech Spells',
  'Blasting Spells',
  'Body Control Spells',
  'Book Spells',
  'Boost Attribute Spells',
  'Buff Spells',
  'Communication and Empathy Spells',
  'Creation Spells',
  'Cyclic Elemental Spells',
  'Death Spells',
  'Divination Spells',
  'Dominions Spells',
  'Dream Spells',
  'Dungeon Fantasy Spells',
  'EM and Weather Spells',
  'Earth Spells',
  'Elemental Body Spells',
  'Elemental Weapon Transformation Spells',
  'Enchantment Spells',
  'Energy Spells',
  'Essential Earth Spells',
  'Ethical Spells',
  'Even More Acid Spells',
  'Even More Unmerged Animal Spells',
  'Fire Spells',
  'Flight Spells',
  'Fluid Spells',
  'Food Spells',
  'Force Spells',
  'Forgotten Realms Spells',
  'Forgotten Spells',
  'Fuel Spells',
  'Fungus Spells',
  'Gate Spells',
  'Glass Spells',
  'Gravity Spells',
  'Gunman Spells',
  'Healing Spells',
  'Hellfire Spells',
  'Ice Spells',
  'Illusion Spells',
  'Illusion and Creation Spells',
  'Incarnum Spells',
  'Individual Spell',
  'Infravision Spells',
  'Knowledge Spells',
  'Lava Spells',
  'Least Spells',
  'Light & Darkness Spells',
  'Light Spells',
  'Light and Darkness Spells',
  'Lightning Spells',
  'Limb Spells',
  'Machine Spells',
  'Making & Breaking Spells',
  'Memory Spells',
  'Meta Spells',
  'Metal Spells',
  'Mind Control Spells',
  'Monster Spells',
  'Moon Spells',
  'More Body of (Element) Spells',
  'Movement Spells',
  'Movement and Communication Spells',
  'Necromantic Spells',
  'New Poison Spells',
  'New Prismatic Spells',
  'New Spell Variants',
  'Old Spells',
  'Personal Air Spells',
  'Personal Animal Spells',
  'Personal Body Control Spells',
  'Personal Earth Spells',
  'Personal Fire Spells',
  'Personal Food, Force, Fungus Spells',
  'Personal Gate and Gravity Spells',
  'Plant Spells',
  'Plasma Spells',
  'Poison Spells',
  'Possession Spells',
  'Power Spells',
  'Psychic Spells as Normal Spells',
  'Radiation Spells',
  'Raw Magic Spells',
  'Revised Elemental Spells',
  'Serpentine Spells',
  'Space Spells',
  'Spells of the Past',
  'Spirit Spells',
  'Stealth Spells',
  'Steam Spells',
  'Technological Spells',
  'Time Spells',
  'Transformation Spells',
  'Unmerged Spells',
  'Water Spells',
  'Weather Spells',
  'Winged Folk Spells',
  'Yellow Spells',
  'Zombie Spells',
];
