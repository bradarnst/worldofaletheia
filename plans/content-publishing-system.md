...
export function shouldIncludeContent(
  ...
  if (status === 'secret') return false; // Always exclude until auth
  if (status === 'draft') return buildMode === 'development';
...
// Status enum
const statusSchema = z.enum(['draft', 'published', 'secret']);
...
const sentientsCollection = defineCollection({
  ...
  schema: z.object({
    ...baseFrontmatter,
    sentientType: z.enum(['npc', 'deity', 'historical', 'player-character']).optional(),
...
│   │   │           └── [...slug].astro
│   │   │
│   │   ├── gurps/                   # Existing GURPS section
│   │   │   └── ... - existing structure
│   │   │
│   │   ├── search.astro             # Search page
│   │   │
│   │   └── api/                     # API routes - future auth
│   │       └── auth/
│   │           └── [...auth].ts
│   │
│   ├── layouts/
...
│   │
│   ├── utils/
│   │   ├── content-filter.ts        # Build-time content filtering
